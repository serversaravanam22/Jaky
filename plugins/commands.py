    ...
    try:
        ...
        if chat_id in user_states:
            ...
            # Awaiting title
                ...
                try:
                    file_info = [
                        f"》{user_states[chat_id]['file_sizes'][i]} : {await short_link(f'https://t.me/{temp.U_NAME}?start=aNsH_{file_id[0]}')}"
                        for i, file_id in enumerate(user_states[chat_id]["file_ids"])
                    ]
                    file_info_text = "\n\n".join(file_info)
                    
                    stream_links_info = [
                        f"》{user_states[chat_id]['file_sizes'][i]} : {await short_link(stream_link[0])}"
                    ...
                    summary_message = (
                        ...
                        f"**𓆩🔻𓆪 Sᴛʀᴇᴀᴍ/Fᴀsᴛ ᴅᴏᴡɴʟᴏᴀᴅ 👇**\n\n"
                        f"**{stream_links_text}**\n\n"
                        f"**✅ Note : [Hᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ]({HOW_TO_POST_SHORT})👀**\n\n"
                        f"**⚡ 𝐉𝐨𝐢𝐧 ➟ : <a href=\"https://t.me/Movieprovidergroups\"><b>Mᴏᴠɪᴇs Rᴇǫᴜᴇsᴛ 𝟸𝟺×𝟽</b></a>**\n\n"
                    ...
                    await message.reply(f"Error occurred while creating the summary message: {e}")
        else:
            return
    except Exception as e:
        await message.reply(f"Error occurred: {e}")
